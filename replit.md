...
- **February 2026:** Unified task management — removed separate Reminders page and consolidated all tasks into a single Inbox view. Inbox now fetches both action items (/api/actions) and personal reminders (/api/personal/reminders) with source-aware mutations (source: 'meeting' | 'quickadd' | 'reminder'). Quick Add "Reminders" destination removed. Planner and Capture updated to reference Inbox instead of Reminders. All /app/reminders URLs redirect to /app/inbox.