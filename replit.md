...
- **February 2026:** Added drag-and-drop reordering using @dnd-kit library. Custom list pages support drag-to-reorder items with optimistic UI updates. Tasks page has a "Custom" sort option enabling manual drag reordering by position field. Backend endpoints POST /api/tasks/reorder (scoped by userId) and POST /api/lists/:id/reorder provide atomic transaction-based position updates. Cross-list movement uses existing "Move to list" action menu in task detail page. Legacy multi-table types (reminder/actionItem) removed from list page in favor of unified task model.
